  ...
  class DailySalesController < Admin::ApplicationController
    ...
    def index
      ...
      if search_term.present?
        ...
        resources = DailySale.where(id: resource_ids).includes(:sales_category).page(params[:_page]).per(records_per_page)
      else
        # Fallback to showing all resources if no search term is provided
        resources = DailySale.includes(:sales_category).page(params[:_page]).per(records_per_page)
      end
